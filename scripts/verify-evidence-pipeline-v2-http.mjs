...
async function startServer({ port, mode, canaryUsers = "" }) {
  const output = [];
...
async function verifyOffMode() {
  const runtime = await startServer({ port: 5212, mode: "off" });
  try {
...
console.log("Evidence pipeline V2 HTTP: off mode and canary guard passed.");
