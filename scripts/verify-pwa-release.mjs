...
check(index.includes(`account-shell.js?v=${version}`), "index.html does not reference the current account shell version.");
...
check(serviceWorker.includes("NETWORK_ONLY_PATHS") && serviceWorker.includes('"/product-shell.js"') && serviceWorker.includes('"/account-shell.js"') && serviceWorker.includes('"/app.js"') && serviceWorker.includes('cache: "no-store"'), "service worker must bypass caching for app shell files.");
check(productShell.includes("global.VibeProductShell = Object.freeze"), "product shell module contract is incomplete.");
check(accountShell.includes("global.VibeAccountShell = Object.freeze"), "account shell module contract is incomplete.");
