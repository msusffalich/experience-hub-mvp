...
check(files.index.includes(`account-shell.js?v=${version}`), "index.html is not loading the current account shell.");
...
check(files.serviceWorker.includes("NETWORK_ONLY_PATHS") && files.serviceWorker.includes('"/product-shell.js"') && files.serviceWorker.includes('"/account-shell.js"') && files.serviceWorker.includes('"/app.js"') && files.serviceWorker.includes('cache: "no-store"'), "app shell files must bypass the service worker cache.");
...
check(files.accountShell.includes("global.VibeAccountShell = Object.freeze"), "account-shell.js does not expose the stable account contract.");
