...
assert(files.server.includes("/api/integration/oura/preflight") && files.server.includes("/api/integration/oura/diagnostic-connect") && files.server.includes("diagnostic_redirect_ok"), "Server must expose a public Oura diagnostic flow that does not require Supabase session.");
...
assert(files.app.includes("preserveSessionOnAuthFailure") && files.app.includes("state.pendingAuthReturn = \"oura\""), "Oura connector flow must not expire the global Vibe session on auth failure.");
...
assert(files.publicationPdf.includes("channel_studio_cards") && !files.publicationPdf.includes("*channel_studio_cards(draft)"), "Publication ReportLab PDF must keep channel studio logic available without exporting it as public PDF clutter.");
...
assert(
  files.server.includes("callMobileAssistantMessages")
    && files.server.includes("callOpenAIAssistantMessages")
    && files.server.includes("OPENAI_ASSISTANT_MODEL")
    && files.server.includes("MOBILE_ASSISTANT_PROVIDER")
    && files.server.includes("MOBILE_ASSISTANT_PROVIDER_TIMEOUT_MS")
