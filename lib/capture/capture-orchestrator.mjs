...
export function createCaptureOrchestrator({
  ...
  async function accept(input = {}) {
    ...
    const existing = await stageCall(
      "catalog",
    ...
    try {
      ...
      if (command.bytes.length > 0) {
        ...
        if (!(await stageCall(
          ...
          await stageCall("storage", "capture_storage_write_failed", () => storage.put(storagePath, command.bytes, {
            checksum: command.checksum,
      ...
      const record = await stageCall("catalog", "capture_catalog_write_failed", () => catalog.upsert({
        captureId: command.captureId,
        ownerUserId: command.ownerUserId,
      ...
      const verified = await stageCall(
        "catalog",
...
async function stageCall(stage, code, action) {
  try {
    return await action();
  } catch (error) {
